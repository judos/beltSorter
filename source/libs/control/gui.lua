...
-- storage.gui.playerData[$playerName].openGui = $(name of opened entity)
...
local function handleEvent(uiComponentIdentifier, player)
    ...
    if eventIsForMod == modName then
        ...
        info("unknown gui event occured: " ..
            serpent.block(uiComponentIdentifier) ..
            ". If this component belongs to your mod it should contain \"$modName.\" as the beginning of the component name.")
    end
end
...
function gui_tick()
    ...
    for _, player in pairs(game.players) do
        if player.connected and (player.opened_gui_type == defines.gui_type.none or player.opened_gui_type == defines.gui_type.entity) then
            ...
            if storage.gui.playerData[playerName] == nil then storage.gui.playerData[playerName] = {} end
            local playerData = storage.gui.playerData[playerName]
